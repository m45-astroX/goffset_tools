...
def get_photons_array(energy_array, exposure)
    photons_array = []
    num_bins = energy_array.size - 1
    num_bins.times do |i|
        energy = 0.5*(energy_array[i]+energy_array[i+1])
        de = energy_array[i+1]-energy_array[i]
        rate = spectrum(energy) * de
        photons = rate * exposure
        photons_array << photons
    end
...
def run_simulation(num, random, output)
    ...
    sim.set_database(detector_configuration: "./aokiSet/database/detector_configuration.xml",
                   detector_parameters: "./aokiSet/database/detector_parameters.xml")
    sim.set_gdml "./aokiSet/database/mass_model.gdml"
    ...
    sim.set_primary_generator :AEObservationPrimaryGen, {
        ...
        arf_filename: "./arf/flat_dummy.arf",
        pixel_size: 0.048,#mm
        num_pixel_x: 320,
        num_pixel_y: 640,
        exposure: 21890.0,
