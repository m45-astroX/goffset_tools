...
int main ( int argc, char *argv[] ) {
    ...
    if ( argc != 6 ) {
        fprintf(stdout, "Usage : ./searchBestSimulation\n");
    ...
    float bestdata[8];  // Format : noise(0) a(1) b(2) c(3) a_err(4) b_err(5) c_err(6) chisquare(7)
    ...
    for (int i=0; i<linenum_increment_simdbfile; i++) {
        ...
        // 現在登録されているChi-squareよりも、今回計算したChi-squareが小さければ
        // 現在の計算結果をベストデータとして登録する
        if ( chiSquare < bestdata[7] ) {
            #ifdef DEBUG
            fprintf(stdout, "chiSquare < bestdata[7] : %f %f\n", chiSquare, bestdata[7]);
            #endif
            // SIMDB format : noise a a_err b b_err c c_err
            // Array format : noise(0) a(1) a_err(2) b(3) b_err(4) c(5) c_err(6) chisquare(7)
            bestdata[0] = simdb_noise[i];
            bestdata[1] = simdb_a[i];
            bestdata[2] = simdb_a_err[i];
            bestdata[3] = simdb_b[i];
            bestdata[4] = simdb_b_err[i];
            bestdata[5] = simdb_c[i];
            bestdata[6] = simdb_c_err[i];
            bestdata[7] = chiSquare;
        }

        #ifdef DEBUG
        fprintf(stdout, "SIMDB number = %d\n", i);
        fprintf(stdout, "chiSquare    = %f\n", chiSquare);
        fprintf(stdout, "\n");
        #endif
    ...
    #ifdef DEBUG_2ND
    // Format : noise(0) a(1) b(2) c(3) a_err(4) b_err(5) c_err(6) chisquare(7)
    fprintf(stdout, "Best noise = %f\n", bestdata[0]);
    fprintf(stdout, "Chi-Square = %f\n", bestdata[7]);
    #endif

    /* Print results */
    for (int i=0; i<8; i++) {
        fprintf(fp_outfile, "%f", bestdata[i]);
        if ( i != 7 ) {
            fprintf(fp_outfile, " ");
        }
        else {
